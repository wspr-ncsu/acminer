...
import org.sag.acminer.phases.acminerdebug.task.CGInacEntryPointWriteFileTask;
...
public class CGInacEntryPointOutputHandler extends AbstractEntryPointOutputHandler {
	
	private final CallGraph cg;

	public CGInacEntryPointOutputHandler(WorkerCountingThreadExecutor exe, Path rootOutputDir, IACMinerDataAccessor dataAccessor){
	...
	protected void onError(EntryPoint ep, WorkerGroup workerGroup, ILogger logger, Throwable t) {
		logger.fatal("{}: Failed to create and start the call graph inaccuracies writing task for for stub '{}' and entry point '{}'.",
	...
	protected WriteFileTask getNewWriteFileTask(EntryPoint ep, ILogger logger) {
		return new CGInacEntryPointWriteFileTask(ep,idCount++,cg,dataAccessor,rootOutputDir,cn,logger);
	}

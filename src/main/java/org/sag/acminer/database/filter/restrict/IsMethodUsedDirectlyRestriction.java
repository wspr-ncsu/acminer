...
@XStreamAlias("IsMethodUsedDirectly")
public class IsMethodUsedDirectlyRestriction extends MethodMatcher implements IRestriction{
	
	public static final String name = "IsMethodUsedDirectly";
	
	public IsMethodUsedDirectlyRestriction(String value) {
		super(value);
	}

	public IsMethodUsedDirectlyRestriction(Op<String> operation) {
		super(operation);
	}
	
	public IsMethodUsedDirectlyRestriction(SootMatcherOpType type, String... values) {
		super(type, values);
	}
	
	public IsMethodUsedDirectlyRestriction(SootMatcherOpType type, boolean ic, String... values) {
		...
		for(INode vn : in) {
			...
			for(ValueBox vb : v.getUseBoxes()) {
				...
				if(use instanceof Local && (localToIgnore == null || !localToIgnore.equals(use)) && applyResInner(vt, vn, use)) {
					//methods can only be referenced by local
					ret.add(vn);
					break;
				}
			}
	...
	public boolean equals(Object o) {
		if(super.equals(o)) {
			return o instanceof IsMethodUsedDirectlyRestriction;
		}
		return false;
	}

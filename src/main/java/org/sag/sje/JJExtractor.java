...
public class JJExtractor {
	...
	public static int run(Path pathToInputDir, Path pathToSystemImgZipIn, Path pathToSystemArchivesZipOut, Path pathToSystemJimpleJarOut, 
			...
			Path pathToWorkingDir, Path pathToAndroidInfo, Path pathToSystemClassJarOut, Path pathToFrameworkPkgsOut, boolean useDextra, 
			boolean disableAllAppsSameTime, boolean disableDumpClasses, String bootClassPath, boolean includeApps, ILogger logger){
		if(useDextra)
		...
		if(pathToFrameworkPkgsOut != null)
			JJOptions.v().setOutput_FrameworkPkgsFile(pathToFrameworkPkgsOut);
	...
	private int run(AndroidInfo ai){
		...
		if(ai.getApi() >= 28) {
			extractor = new VdexExtractorDexExtractor(JJOptions.v().getBootClassPath(), JJOptions.v().getInputDir(), JJOptions.v().getInput_SystemImgZip(),
					...
					JJOptions.v().getLocations(), JJOptions.v().getArchs(), JJOptions.v().getApexLocations(), ai.getApi(),JJOptions.v().getMainLogger());
		...
		List<String> resolvedBootClassPath = null;
		...
		try{
			...
			resolvedBootClassPath = extractor.getResolvedBootClassPath();
		}catch(Throwable t){
		...
		DexToJimpleDecompiler dexDecompiler = new DexToJimpleDecompiler(JJOptions.v().getInputDir(),

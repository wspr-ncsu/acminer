...
public class JimpleJarHandler extends AbstractPhaseHandler {
	...
	private Path pathToFrameworkPkgsOut;
	...
	protected void initInner() {
		...
		this.pathToFrameworkPkgsOut = temp.get(8);
	}
		...
		try {
			if(runJJExtractor) {
				int ret = JJExtractor.run(rootPath, sourceSystemImgZipFilePath, pathToSystemArchivesZipOut, getOutputFilePath(), 
						...
						pathToWorkingDir, sourceAndroidInfoFilePath, pathToSystemClassJarOut, pathToFrameworkPkgsOut, 
						false, true, false, null, false, logger);
				if(ret != 1) {

	...
	protected boolean parseOatFile(String location, Path oatFilePath, Path workingDir, Path outputDir, String arch, Path bootDirForArch) {
		...
		try {
			...
			for(String entry : container.getDexEntryNames()){
				DexBackedDexFile dexFileRef = container.getEntry(entry);
				if(dexFileRef instanceof DexBackedOdexFile){
					...
					OatDexFile oatFile = (OatDexFile)dexFileRef;
					try{
						int oatLevel = oatFile.getContainer().getOatVersion();
						int apiLevel = VersionMap.mapArtVersionToApi(oatLevel);

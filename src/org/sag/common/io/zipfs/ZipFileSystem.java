...
import static org.sag.common.io.zipfs.ZipConstants.*;
import static org.sag.common.io.zipfs.ZipUtils.*;
...
class ZipFileSystem extends FileSystem {
				  ...
				  Map<String, ?> env)  throws IOException
	{
		...
		this.LOOKUPKEY = new IndexNode(null, -1, zc);
	...
	public Iterable<Path> getRootDirectories() {
		return ImmutableList.of(rootdir);
	}
	...
	public Iterable<FileStore> getFileStores() {
		return ImmutableList.of(new ZipFileStore(rootdir));
	}

	private static final Set<String> supportedFileAttributeViews =
			ImmutableSet.of("basic", "zip");
		...
		throws IOException
	{
		...
		try {
			...
			if (e == null) {
				...
				if (inode == null)
					return null;
				e = new Entry(inode.name, inode.isdir, zc);  // pseudo directory
		...
		throws IOException
	{
		...
		try {
			...
			checkParents(dir);
			Entry e = new Entry(dir, Entry.NEW, true, zc);
			e.method = METHOD_STORED;			// STORED for dir
			update(e);
		} finally {
			endWrite();
		...
		throws IOException
	{
		...
		try {
			...
			Entry u = new Entry(eSrc, Entry.COPY, zc);  // copy eSrc entry
		...
		try {						// try to obtain a writelock when the os is
			...
			if (e != null) {
				...
				if (hasAppend) {
					...
					OutputStream os = getOutputStream(new Entry(e, Entry.NEW, zc));
					copyStream(is, os);
					is.close();
					return os;
				}
				return getOutputStream(new Entry(e, Entry.NEW, zc));
			} else {
				if (!hasCreate && !hasCreateNew)
					throw new NoSuchFileException(getString(path));
				checkParents(path);
				return getOutputStream(new Entry(path, Entry.NEW, false, zc));
			}
		} finally {
			endRead();
		}
		...
		throws IOException
	{
		...
		try {
			...
			final Entry u = isFCH ? e : new Entry(path, tmpfile, Entry.FILECH, zc);
			if (forWrite) {
	...
	private void checkParents(byte[] path) throws IOException {
		...
		try {
			while ((path = getParent(path)) != null &&
					path != ROOTPATH) {
				if (!inodes.containsKey(IndexNode.keyOf(path,zc))) {
					throw new NoSuchFileException(getString(path));
				}
			}
		} finally {
	...
	private byte[] initCEN() throws IOException {
		...
		while (pos < limit) {
			...
			IndexNode inode = new IndexNode(cen, pos + CENHDR, nlen, pos, zc);
	...
	IndexNode getInode(byte[] path) {
		...
		return inodes.get(IndexNode.keyOf(path, zc));
	}
	...
	static class IndexNode {
		...
		IndexNode(byte[] name, boolean isdir, ZipCoder zc) {
			name(name);
			this.isdir = isdir;
			this.pos = -1;
			this.zc = zc;
		...
		IndexNode(byte[] name, int pos, ZipCoder zc) {
			name(name);
			this.pos = pos;
			this.zc = zc;
		}
		...
		IndexNode(byte[] cen, int noff, int nlen, int pos, ZipCoder zc) {
			if (cen[noff + nlen - 1] == '/') {
				isdir = true;
				nlen--;
			}
		...
		final static IndexNode keyOf(byte[] name, ZipCoder zc) { // get a lookup key;
			IndexNode key = cachedKey.get();
			if (key == null) {
				key = new IndexNode(name, -1, zc);
				cachedKey.set(key);
			}
			return key.as(name);
		}
		...
		public String toString() {
			if(zc != null && name != null) {
				return zc.toString(name);
			}
			return "";
		}

		IndexNode(ZipCoder zc) {this.zc = zc;}
		IndexNode sibling;
	...
	static class Entry extends IndexNode implements ZipFileAttributes {
		...
		Entry(ZipCoder zc) {super(zc);}

		Entry(byte[] name, boolean isdir, ZipCoder zc) {
			super(zc);
			name(name);
			this.isdir = isdir;
		...
		Entry(byte[] name, int type, boolean isdir, ZipCoder zc) {
			this(name, isdir, zc);
			this.type = type;
		}

		Entry (Entry e, int type, ZipCoder zc) {
			super(zc);
			name(e.name);
			this.isdir	 = e.isdir;
		...
		Entry (byte[] name, Path file, int type, ZipCoder zc) {
			this(name, type, false, zc);
			this.file = file;
		...
		static Entry readCEN(ZipFileSystem zipfs, IndexNode inode)
			throws IOException
		{
			return new Entry(zipfs.zc).cen(zipfs, inode);
		}
	...
	static class ParentLookup extends IndexNode {
		int len;
		ParentLookup(ZipCoder zc) {super(zc);}
		...
		public boolean equals(Object other) {
			...
			if(name == null && oname == null)
				...
				return false;
			return ByteBuffer.wrap(name,0,len).equals(ByteBuffer.wrap(oname,0,oname.length));
		}
	...
	private void buildNodeTree() throws IOException {
		beginWrite();
		try {
			IndexNode root = new IndexNode(ROOTPATH, true, zc);
			IndexNode[] nodes = inodes.keySet().toArray(new IndexNode[0]);
			inodes.put(root, root);
			ParentLookup lookup = new ParentLookup(zc);
			for (IndexNode node : nodes) {
				...
				while (true) {
					...
					parent = new IndexNode(Arrays.copyOf(node.name, off), true, zc);
	...
	/*private static void dumpNodeTree(IndexNode root) {
		ArrayDeque<IndexNode> stack = new ArrayDeque<>();
		stack.push(root);
		while(!stack.isEmpty()) {
			IndexNode cur = stack.pop();
			System.out.println(cur.toString());
			IndexNode child = cur.child;
			while(child != null) {
				stack.push(child);
				child = child.sibling;
			}
